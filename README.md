...
Our selective-resetting method applies to _any_ linear recurrence (diagonal or not, time-variant or not, over real numbers or other fields) computed in parallel via a prefix scan. This repository provides a sample implementation for only one case: non-diagonal linear recurrences over real numbers, either time-variant or time-invariant. The implementation, for PyTorch, is in a single file: [sample_implementation.py](sample_implementation.py).

We will walk through an example to show how to use our sample implementation. Launch a Python interpreter (_e.g._, in a notebook), and execute the following code to compute a non-diagonal linear recurrence $X_t = A_t X_{t-1}$, with initial state $X_0 = I$, in parallel, via a prefix scan:
